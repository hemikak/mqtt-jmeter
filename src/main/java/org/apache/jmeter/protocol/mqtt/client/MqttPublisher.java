...
public class MqttPublisher extends AbstractJavaSamplerClient implements
    ...
    public void setupTest(JavaSamplerContext context) {
        ...
        // Quality
        if (MQTTPublisherGui.AT_MOST_ONCE.equals(context.getParameter("QOS"))) {
            qos = 0;
        } else if (MQTTPublisherGui.AT_LEAST_ONCE.equals(context.getParameter("QOS"))) {
            qos = 1;
        } else if (MQTTPublisherGui.EXACTLY_ONCE.equals(context.getParameter("QOS"))) {
            qos = 2;
        }

        if (MQTTPublisherGui.TEXT_INPUT.equals(messageInputType)) {
            publishMessage = context.getParameter("MESSAGE_VALUE");
        } else if (MQTTPublisherGui.FILE_INPUT.equals(messageInputType)) {
    ...
    public void setupTest(String brokerURL, String clientId, String userName, String password, String clientType) {
        try {
            if (MQTTSubscriberGui.BLOCKING_CLIENT.equals(clientType)) {
                client = new BlockingClient(brokerURL, clientId, false, userName, password);
            } else if (MQTTSubscriberGui.ASYNC_CLIENT.equals(clientType)) {
